...
# ------------------------------------------------------------------------------
def exportar(texto):
    with open('ResultadoA.txt', 'w') as f:
        f.write(texto)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
begin("Exercicio Avaliativo - Centros de Distribução - Parte A") # inicia um modelo
#verbose("true")
...
# FUNÇÃO OBJETIVO
# ------------------------------------------------------------------------------
minimize(sum(DAB[i][j] * x[i,j] * Q[j] for i,j in A))
#-------------------------------------------------------------------------------
# RESTRIÇÕES
# ------------------------------------------------------------------------------
for j in range(n):
...
for i,k in enumerate(C):
  sum((x[i,j] for j in range(n))) <= 100 * y[k] # realizando a distribuição balanceada
#-------------------------------------------------------------------------------
# RESOLVENDO
# ------------------------------------------------------------------------------
solver(int, tm_lim = 3600 * 1000) # LIMITAÇÃO DE TEMPO
solve()
#-------------------------------------------------------------------------------
...
CentMenor = ""
acumulador = []
text = text + "\n Centros e as suas Localidades"
for i,k in enumerate(C):
  if y[k].primal == 1:
    print(f"CD{C[i]} - ({L[k][0]},{L[k][1]}): ", end=" ")
    text = text + f"\nCD{C[i]} - ({L[k][0]},{L[k][1]}): "
    acm = 0
    for j in range(n):
      if x[i,j].primal == 1:
        print(f"CD{C[i]}L{j}({L[j][0]},{L[j][1]})", end="   ")
        text = text + f"CD{C[i]}L{j}({L[j][0]},{L[j][1]})  "
        acm = (DAB[i][j] * Q[j]) + acm
...
print(f"\nValor ótimo/Distancia to  tal percorrida = {vobj():.2f}")
print(f"\nMáximo distancia da soma de cada centos = {max(acumulador, default=0.00):.2f}")
print(f"\nMínimo distancia da soma de cada centos = {min(acumulador, default=0.00):.2f}")

text = text + f'''
\n* * * Dados de Saida * * *
\nValor otimo/Distancia total percorrida = {vobj():.2f}
\nMaximo distancia da soma de cada centos = {max(acumulador, default=0.00):.2f}
\nMinimo distancia da soma de cada centos = {min(acumulador, default=0.00):.2f}
